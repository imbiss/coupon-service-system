...
if (isset($_GET["kata"])) {
    ...
    $controllerName=isset($paramList[0])?($paramList[0]):$defaultController;
    $actionName = isset($paramList[1])?(empty($paramList[1])?"index":$paramList[1]):"index";

} else {
    // direct call without ReWrite
    $controllerName = isset($_REQUEST["controller"]) ? ($_REQUEST["controller"]) : $defaultController;
    $actionName = isset($_REQUEST["action"]) ? ($_REQUEST["action"]) : "index";
}


$d = new dispatcher();
$d->setControllerType(dispatcher::PARTNER)
