...
function loadModel($name) {
	//$file = ROOT.'/models'.DS.strtolower($name).'.php';
    $file = ROOT.'/models'.DS.trim($name).'.php';
	if (file_exists($file)) {
		require_once $file;
		return;
	}	
	throw new Exception('basics: loadModel: cant find Model ['.$name.']' . ' file:' . $file);
}
...
function vendor($name) {
	$args= func_get_args();
	foreach ($args as $arg) {
		$file = ROOT.'vendors'.DS.$arg.'.php';
		if (file_exists($file)) {
			require_once (ROOT.'vendors'.DS.$arg.'.php');
		} else {
			throw new Exception( 'Cant vendor() ' . $arg . "($file)");
		}
	}
}
